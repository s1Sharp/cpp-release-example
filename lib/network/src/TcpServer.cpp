...
bool TcpServer::listen(const boost::asio::ip::tcp &protocol, uint16_t port) {
    try {
        ...
        std::cerr << "TcpServer::Listen() caught exception: " + static_cast<std::string>(e.what())  << std::endl;
        return false;
    }
    return true;
}
...
void TcpServer::send(int connectionId, const char *data, size_t size) {
    if (m_connections.count(connectionId) == 0) {
        std::cerr << "TcpServer::send() found error: connectionId not found" << std::endl;
        return;
    }
...
void TcpServer::doAccept() {
    ...
    m_acceptor.async_accept([this](const auto &error, auto socket) {
        if (error) {
            std::cerr << "TcpServer::doAccept() found error: " + error.message() << std::endl;
            m_isAccepting = false;
            return;
        } else {
            ...
            std::cout << "TCP Server accepted connection: " + std::to_string(m_connectionCount) << std::endl;
...
void TcpServer::onConnectionClosed(int connectionId) {
    ...
    if (m_connections.erase(connectionId) > 0) {
        std::cout << "TCP Server removed connection: " + std::to_string(m_connectionCount) << std::endl;
