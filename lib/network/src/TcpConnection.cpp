...
void TcpConnection::close() {
    try {
        ...
        std::cerr << "TcpConnection::Close() caught exception: " + static_cast<std::string>(e.what()) << std::endl;
        return;
    }
...
void TcpConnection::doRead() {
    ...
    m_socket.async_read_some(
        buffers, [this, self](const auto &error, auto bytesTransferred) {
            if (error) {
                std::cerr << "TcpConnection::doRead() found error: " + error.message() << std::endl;
                return close();
            }
...
void TcpConnection::doWrite() {
    m_isWritting = true;
    auto self = shared_from_this();
    m_socket.async_write_some(m_writeBuffer.data(),
        [this, self](const auto &error, auto bytesTransferred)
    {
        if (error) {
            std::cerr << "TcpConnection::doWrite() found error: " + error.message() << std::endl;
            return close();
        }
        std::lock_guard<std::mutex> guard{m_writeBufferMutex};
